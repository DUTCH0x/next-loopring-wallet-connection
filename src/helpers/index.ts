...
export const shortenAddress = (hexString: string) => {
  if (!hexString) return '';
...
export const getFormattedAmount = (
  ...
  const formattedPrice = ethers.BigNumber.from(amount);
  return ethers.utils.formatUnits(formattedPrice, decimals);
};;

export const getFormattedDate = (timestamp: number, timezoneIdentifier: string) => {
...
export const shortenNFTId = (hexString: string) => {
  const prefix = hexString.slice(0, 4);
  const suffix = hexString.slice(-4);
  return `${prefix}...${suffix}`;
};
...
export const getLastNPeriods = (
  ...
  n = 12,
  format = 'MMM YYYY',
  duration: moment.unitOfTime.DurationConstructor = 'months'
) => {
  const today = moment();
  const months: { [monthyr: string]: any } = {};
  for (let i = 0; i < n; i++) {
    const period = today.clone().subtract(i, duration).format(format);
    months[period] = { ...monthValue, Date: period };
  }
  return months;
};

export const generateAccountAvartarInBase64 = (account: string) => {
  try {
    const data = new Identicon(account, { format: 'png' }).toString();
